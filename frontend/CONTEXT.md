...
- **`useTodayData.ts`** — the one data hook: `useLiveQuery` (from `dexie-react-hooks`, first real usage in the repo) over `src/db/dayView.ts`'s `getDayItems`, plus a minute-tick clock. Writes via `repo.ts` (`markReminder`, `startWorkSession`) auto-refresh it — no manual invalidation anywhere in this feature. Also derives the earlier/up-next time buckets (anchored on `strict_time` for strict, `soft_start` for soft, and **`soft_end` for `random`** — so a not-yet-fired surprise stays in "up next" for its whole window; a `random` row's `timeLabel` is the window *range* rendered on two lines, its exact fire minute stays hidden) and the header counts — `toGoCount` excludes both `done` and `skipped` ("Missed it") occurrences, so a dismissed item isn't counted as still outstanding (done + toGo need not sum to the total). `streak` is a **hardcoded stub** — real derived-streak calculation isn't built (see `db/CONTEXT.md`).
...
- **`ReminderRow.tsx`** — a reminder only renders dimmed/checked once `completion.status === "done"`. A reminder whose time has passed with no completion still renders at full opacity with a tappable empty circle, **not** styled as a failure — root `CLAUDE.md` bans punishing "missed" states, so a late reminder stays actionable rather than showing as a failure. The **one** exception is a user-chosen **"Missed it"** dismissal (`completion.status === "skipped"`): dimmed + strikethrough. That's a deliberate act, not the banned system-missed styling. `done` and `missed` are mutually exclusive (one completion; status `done | skipped`). The `time` cell is `whitespace-pre-line` so a `random` reminder's two-line window range (`"9:00 AM\n–5:00 PM"`, built in `useTodayData`) stacks without widening the fixed column; single-time rows are unaffected.
