...
**Data layer — `src/db/stats.ts`** (pure Dexie reads, aggregated in memory; see `db/CONTEXT.md`). Loads the user's rows once, indexes occurrence state by `activity_id|date`, then derives everything. Windows: "this week" = **rolling 7 days ending today**, delta vs the prior 7; heatmap all-time. **Archived rule (implemented):** amount stats (focus banked / trend / heatmap) include archived history; rate stats (showed-up ÷ planned / adherence / most-avoided) are active-only; archived rows appear as greyed "history only" (only if they have logged history). Heatmap buckets from `work_sessions.started_at` local weekday+hour (reminder `completed_at` is tap-time, not do-time, so reminders are excluded); timestamps are UTC ISO → `new Date()` renders them device-local, which is what we bucket by. Session/log dates use a fixed English `MMM D` (locale-stable, app is English-only). Week-strip marks *today* as `pending` (not a miss) since it's still actionable; most-avoided has a min-planned floor (2). **Bundle note:** `recharts` pushes the main chunk past Vite's 500 KB warning — code-splitting `/stats` (`React.lazy`) is a reasonable follow-up (lazy chunks still precache for offline).
