...
- `recurrence.ts` — pure recurrence/time helpers (`todayLocal`, `parseLocalDate`, `weekdayOf`, `recursOn`, plus the 'HH:MM' local wall-clock helpers `nowTimeLocal`/`nextHourLocal`/`defaultStartTimeLocal`/`hourAfterLocal` — used to default and floor same-day reminder/soft-window times so they can't be set in the past), shared by `dayView.ts`, `repo.ts`, and the create-activity form.
