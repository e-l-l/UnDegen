...
| Navigation / routing | react-router v7 (declarative `BrowserRouter`) | Introduced for the Stats page. Routes `/today`, `/focus` (mobile-only — redirects to `/today` at ≥`lg`), `/stats`, `/stats/:activityId`, `/you`, catch-all → `/today`. Nav entries: `MobileTabBar` = Today/Focus/Stats/You; `DesktopIsland` = Today/Stats/You (no Focus on desktop). Screens own their chrome (no shared layout route — Today's desktop rail differs from Stats'). SW notification-click → `/today`; `frontend/vercel.json` SPA rewrite so deep links don't 404. **Stats + You are built** — both end-to-end. Stats (`frontend/src/features/stats/` + `frontend/src/db/stats.ts`); You (`frontend/src/features/you/`, styled from the design hand-off — see `frontend/CONTEXT.md`) owning profile (identity as title, **no avatar**) / reallow-notifications / app-version+update / sign-out (sign-out clears Dexie, confirming if `syncQueue` has unflushed writes). `recharts` v3 installed (first usage, `FocusTrend` only; heatmap/flake hand-rolled) |
| Day switcher / viewed day | Review-only, real today ±7, shared context | Today **and** Focus step through days (chevrons only, no calendar/swipe) within real today ±7 to see that day's view. Other days are **review-only** — real today is the only day that accepts writes (no completing/skipping/starting/deleting off-today); no retroactive writes (see Open Questions). The **viewed day** lives in `SelectedDayProvider` (context above the routes) so it persists across Today↔Focus; resets to today on cold start + notification tap (`sw.ts` posts `notification-click`). Off-today: flat reminder list (no NOW/Earlier/Up-next), relative title (Yesterday/Tomorrow/weekday) + neutral `X of Y done`, read-only rows/cards, long-tasks show that day's logged time / `Planned`. No `db/` change — `dayView.deriveState` already resolves past→`missed`/future→`pending` for any date. Plumbing lives in `frontend/src/features/today/`; UI spec in that folder's `DESIGN_HANDOFF_day_switcher.md` |
...
- **Un-archive / restore** — deleting a task is one-way from the UI. "Delete entire event" sets `archived`; "delete this day" appends to `exception_dates`. Nothing reverses either (no undo toast, no manage/archived screen), so an archived activity or a removed day can't be brought back in-app yet. Restoring needs a manage/archived surface — the **You** screen exists now but doesn't host it (declined for v0).
