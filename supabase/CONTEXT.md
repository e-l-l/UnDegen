...
- `migrations/0009_activity_revisions.sql` — date-effective schedule/configuration with one backfilled initial revision, immutable activity type/start enforcement, parent-type validation, owner RLS, explicit grants, server-managed save timestamps, and a trigger keeping the legacy mirror aligned with the last revision upsert (ADR 0004).
- `functions/send-notifications/` — the Web Push sender (Deno). `index.ts` (handler) + `schedule.ts` (pure due-ness/slot logic, mirrors the frontend's `recurrence.ts` — its `recursOn` skips a date in `exception_dates`, and `index.ts` already filters `archived = false` in the activities query) + `schedule.test.ts` (`deno test`). Runtime packages in `index.ts` use explicit `npm:` specifiers so Supabase Dashboard/function-only bundles do not depend on the parent `functions/deno.json` import map.
